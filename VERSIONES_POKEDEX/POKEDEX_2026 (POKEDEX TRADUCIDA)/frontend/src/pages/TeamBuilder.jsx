...
function TeamBuilder() {
  ...
  const saveTeam = async () => {
    if (team.length === 0 || !teamName) {
      alert('¡Dale un nombre a tu equipo y agrega al menos 1 Pokémon!');
      return;
    }
    ...
    try {
      ...
      // Guardar datos completos del equipo para la batalla
      ...
      alert('¡Equipo guardado correctamente! Listo para la batalla.');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <div className="glass-panel" style={{ marginBottom: '2rem' }}>
        <h1 className="title">Tu Equipo</h1>
        <div className="flex-center" style={{ gap: '1rem', marginBottom: '1rem' }}>
          ...
          <button className="btn" onClick={clearTeam}>Limpiar</button>
        </div>

        {team.length === 0 ? <p style={{ textAlign: 'center' }}>No hay Pokémon seleccionados. ¡Elige hasta 6!</p> : null}
      ...
      <div className="glass-panel">
        <h2 className="title" style={{ fontSize: '2rem' }}>Pokémon Disponibles</h2>
